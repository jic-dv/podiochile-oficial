...
import { WHATSAPP_NUMERO, WHATSAPP_DISPLAY, CORREO } from "@/features/quotes/lib/quotes";
import type { Servicio } from "@/features/services/schemas/service.schema";
import {
  IconWhatsapp, IconCorreo, IconHorario,
} from "@/shared/lib/icons";

export default function ContactoSection({ servicios }: { servicios: Servicio[] }) {
  const { t } = useI18n();
    ...
    <Section id="contacto" aria-labelledby="contacto-heading">
      <div className="mx-auto grid max-w-6xl items-start gap-12 lg:grid-cols-2 lg:gap-16">
        <Reveal>
          ...
          <h2
            id="contacto-heading"
            className="mb-5 font-display text-3xl font-bold text-[var(--color-text-primary)] sm:text-4xl"
          >
            {t.contacto.titulo}
          </h2>
          <p className="mb-8 text-lg leading-relaxed text-[var(--color-text-body)]">
          ...
          <ul role="list" className="space-y-5">
            {canales.map(({ Icon, titulo, valor, href, externo }) => (
              <li key={titulo} className="flex items-start gap-4">
                ...
                <div>
                  <p className="font-semibold text-[var(--color-text-primary)]">{titulo}</p>
                  {href ? (
                    <a
                      href={href}
                      {...(externo ? { target: "_blank", rel: "noopener noreferrer" } : {})}
                      className="text-[var(--color-text-body)] transition-colors hover:text-[var(--color-brand)]"
                    >
                      {valor}
                    </a>
                  ) : (
                    <p className="text-[var(--color-text-body)]">{valor}</p>
                  )}
                </div>
              </li>
            ))}
        ...
        <Reveal delay={120}>
          ...
          <Suspense fallback={<div className="h-[36rem] rounded-[var(--radius-2xl)] border border-[var(--color-border)] bg-[var(--color-surface-raised)]" />}>
