...
export default function BannerCookies() {
  ...
  return (
    ...
    >
      <div
        className={cn(
          "mx-auto w-full max-w-3xl rounded-[var(--radius-xl)] p-5",
      ...
      >
        ...
        <p
          id="cookies-texto"
          className="mt-1.5 text-sm leading-relaxed text-[var(--color-text-body)]"
        >
        ...
        >
          {detalleAbierto ? t.cookies.bannerVerMenos : t.cookies.bannerVerDetalle}
          <IconChevronDown
            className={cn(
        ...
        <div
          ...
          className="mt-3 overflow-x-auto rounded-[var(--radius-lg)] border border-[var(--color-border)]"
        >
          <table className="w-full min-w-[30rem] border-collapse text-left text-xs">
            ...
            <thead>
              <tr className="border-b border-[var(--color-border)] bg-[var(--color-surface-muted)]">
                <th scope="col" className="px-3 py-2 font-semibold text-[var(--color-text-primary)]">
                  {t.cookies.colClave}
                </th>
                <th scope="col" className="px-3 py-2 font-semibold text-[var(--color-text-primary)]">
                  {t.cookies.colQue}
            ...
            <tbody>
              {filas.map((f) => (
                <tr key={f.clave} className="border-b border-[var(--color-border-subtle)] last:border-0">
                  <td className="px-3 py-2 font-mono text-[11px] text-[var(--color-text-primary)]">
                    {f.clave}
                  </td>
                  <td className="px-3 py-2 text-[var(--color-text-body)]">{f.que}</td>
                  <td className="px-3 py-2 text-[var(--color-text-body)]">{f.para}</td>
                  <td className="px-3 py-2 text-[var(--color-text-muted)]">{f.dura}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <p className="border-t border-[var(--color-border)] bg-[var(--color-surface-muted)] px-3 py-2.5 text-xs leading-relaxed text-[var(--color-text-muted)]">
        ...
        <div className="mt-4 flex flex-col gap-2.5 sm:flex-row">
          <Button size="md" className="flex-1" onClick={() => decidir("aceptado")}>
            {t.cookies.aceptar}
          </Button>
          <Button
            variant="outline"
        ...
        <p className="mt-3 text-xs text-[var(--color-text-muted)]">
          {t.cookies.bannerPie}{" "}
          <Link href="/cookies" className="font-medium text-[var(--color-brand)] underline underline-offset-2">
            {t.footer.cookies}
          </Link>
          {" · "}
          <Link href="/privacidad" className="font-medium text-[var(--color-brand)] underline underline-offset-2">
            {t.footer.privacidad}
          </Link>
        </p>
      </div>
